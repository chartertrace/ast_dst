...
// Go type and doc comment. Embedded fields are kept (their type name is the var).
func (p *pkg) structVars(st *ast.StructType) []StateVar {
	...
	for _, f := range st.Fields.List {
		...
		if len(f.Names) == 0 { // embedded field: the type is the name
			out = append(out, StateVar{Name: lastSegment(typ), Type: typ, Doc: doc, Loc: p.loc(f.Pos())})
			continue
		}
		for _, name := range f.Names {
			out = append(out, StateVar{Name: name.Name, Type: typ, Doc: doc, Loc: p.loc(name.Pos())})
		}
	}
	return out
}
